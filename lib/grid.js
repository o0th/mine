    ...
    irons = irons.map((iron) => {
      ...
      if ((current.name === 'mine' || current.name === 'belt') && current.output === 'right') {
        ...
        if (next.input === 'left') {
          ...
          const opts = { repeat: false, ease: 'linear' }
          const animation = { position: { x: iron.graphic.x + 50, y: 0 }, duration: 1000 }
          ease.removeEase(iron.graphic)
          ease.add(iron.graphic, animation, opts)
        }
      }
      ...
      if ((current.name === 'mine' || current.name === 'belt') && current.output === 'up') {
        ...
        if (next.input === 'down') {
          ...
          const opts = { repeat: false, ease: 'linear' }
          const animation = { position: { x: 0, y: iron.graphic.y - 50 }, duration: 1000 }
          ease.removeEase(iron.graphic)
          ease.add(iron.graphic, animation, opts)
        }
      }
